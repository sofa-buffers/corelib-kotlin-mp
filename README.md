...
Nothing about the multiplatform target set costs speed on the JVM: the hot paths
are the same ones the reference Java port uses — an eight-byte store per varint, a
cursor advanced over a contiguous buffer, no per-field allocation — reached from
common code through a per-target `expect`/`actual` for the little-endian word
access alone (byte-array `VarHandle`s on the JVM, indexed shifts elsewhere).
Measured the way that survives a busy machine — instructions retired per op under
Callgrind — it lands **within about 1 % of `corelib-java`** on BENCH_SPEC's shared
workloads, and ahead of it on one (see [Benchmarks](#benchmarks)).
...
each in `build.gradle.kts`; they are left out of the default target set because the
CI runner is Linux and a target nobody builds is a target nobody tests.
...
`sofab` namespace (CORELIB_PLAN §6) under the organisation's group, exactly as the
Java port spells it.

### Requirements

- **Kotlin 2.4+** and **Gradle 8.14.4+** to build (the wrapper pins both).
...
| Goal | How |
|------|-----|
| Streaming **out** | `OStream` writes into a small caller-owned `ByteArray` and hands each full buffer to a `FlushSink`, so a message can exceed the buffer — and RAM. `MIN_OUTPUT_BUFFER` is **1**: no atomic unit has to land contiguously, and any size at or above it produces byte-identical output. |
| Streaming **in** | `IStream.feed` accepts arbitrarily small chunks; a message may split at any byte boundary, and string / blob payloads arrive in pieces. Malformed bytes throw `SofabException(INVALID_MSG)`; running out of bytes mid-field is **not** an error — `feed` suspends and resumes. `status` afterwards tells a `COMPLETE` message from a truncated `INCOMPLETE` one; it never throws and there is no finish/finalize step (CORELIB_PLAN §5.2). A rejection is **terminal** and `reset()` is what clears it. |
| Chunking costs only what straddles | Whole fields are decoded by a cursor advanced over the buffer; only the one construct that would run past the end of the supplied bytes goes through the resumable byte-at-a-time machine, and the rest of the chunk goes straight back to the bulk path — inside an array as much as between fields. A boundary in a 200-element array costs one element, not the remainder (`StreamingTest` asserts exactly that, by counting the bytes the machine saw). |
...
generated-code path that wraps them. Kotlin has no checked exceptions:
`SofabException` is a `RuntimeException` and is documented, not declared.
...
A sink that *takes* the buffer instead of copying out of it — a zero-copy transport
— must hand the encoder a replacement before it returns, and the cursor then starts
at that installation's offset. That is also how a sink reserves framing-header room
in **every** flushed unit, since the offset belongs to the installation and is
consumed by the flush it was made in:
...
rather than after the payload assembles — which is what keeps the verdict from
depending on where the chunk boundaries fell.

### Deserialize stream

Because all parse state lives inside `IStream`, feed it whatever chunks arrive —
even one byte at a time — and it resumes across boundaries. String and blob
payloads arrive as one or more chunks tagged with the field `total` length and the
chunk `offset`, so a payload never has to be held in one piece:
...
A fed chunk is **borrowed only for the duration of the call** (CORELIB_PLAN §6):
the decoder keeps no view into it, so the array above may be refilled the moment
`feed` returns.
...
closed name set (CORELIB_PLAN §6.1.1) — `encode()` and `decode(bytes)` for the
one-shot 90 % case, `serialize(ostream)` and `deserialize(...)` for the streaming
pair underneath, and `decoder()` for the incremental reader:
...
streaming one, which is why a message that streams and a message that fits in a
buffer produce identical bytes. Generated code is also where the **schema** rules
...
- **Output buffer (encoding).** The caller owns and sizes it; the encoder writes
  ...
  returning (CORELIB_PLAN §5.1). Which of the two happened is stated by that call
  and by nothing else. The **start offset belongs to the installation, not to the
  buffer**: it is consumed by the flush it was made in, so a sink wanting header
  room in *every* unit re-arms it on each flush (passing the same array again is a
  new installation like any other).
  ...
  **without** a sink is subject to no minimum: no flush can occur, so it either
  ...
  stays exact.
  **Pass-through is not implemented** (CORELIB_PLAN §5.1 makes it a MAY): a sink
  never receives memory other than the buffer it was given, so a sink that retains
  what it is handed only has to reckon with the encoder reusing that buffer.
  ...
  **seven scratch bytes** just past the write position. They are never part of the
  ...
  array, and is **always strict** (MESSAGE_SPEC §8): a Kotlin `String` is a Unicode
  string type, so the only value it can hold that is not well-formed UTF-8 is an
  unpaired surrogate, and such a string is refused with `ARGUMENT` **before** any
  byte is written, never lossily replaced. The byte-container door,
...
- **Input buffer (decoding).** `feed` runs a cursor over the caller's array,
  **aliasing** it. Scalars and floats are passed by value (`Long` / `Double` / raw
  ...
  retains bytes must copy the range. This port has one decode surface, `feed`, so
  the rule is uniform: there is no one-shot variant that hands out views outliving
  the call, and a caller may reuse or discard a chunk the instant `feed` returns.
  The decoder allocates nothing per field; its only heap use is an 8-byte carry
  array, allocated the first time a float payload straddles a chunk boundary and
  reused thereafter.
...
./gradlew koverHtmlReport koverVerify   # coverage report, and the >90% gate of §7.3
```

The suite is the shared conformance vectors plus the test kinds CORELIB_PLAN §7.2
requires. `assets/test_vectors.json` is read straight from `assets/` — the copy the
repo must carry anyway, never a duplicate that could drift — and every vector is
driven through encode, chunked encode (1/3/7-byte buffers with a flush sink),
decode, chunked decode (1/3/7-byte feeds), skip-ids and roundtrip. Alongside it:
malformed-input and truncation tables (including the `fixlen_word` cut after a byte
whose settled low bits already name a reserved subtype — `INCOMPLETE`, because §4.1
forbids evaluating part of an incomplete varint), tolerance cases (non-minimal
varints, a sequence end whose id is not zero), the chunk-lifetime scrub, the
taking- and copying-sink handovers, the `arrayBulk` narrowing rules, and the fp32
signaling-NaN round trip that the JSON vectors cannot express.
...
- **`run_callgrind.sh`** reports **instructions retired per op** under Callgrind —
  ...
  across machines and against the sibling ports. There is no JIT-compiled
  `run_<workload>` symbol to toggle collection on, so it runs each workload at two
  rep counts and subtracts, which cancels JVM startup, class loading and JIT cost
  exactly.

### Measured here

Instruction counts are deterministic and machine-independent, so unlike MB/s they
are worth writing down. Taken with `bench/run_callgrind.sh` on Temurin 21.0.12,
alongside `corelib-java` driven through **its** Callgrind tool on the same host,
JVM and rep deltas:
...
and ahead of it on the composite encode. The Kotlin abstractions that could have
cost something here — a common-code codec, an `expect`/`actual` seam on every
multi-byte load, an interface with default methods on the decoder's hottest edge —
are compiled away: the seam becomes the same intrinsified `VarHandle` access the
Java port uses, and `-jvm-default=no-compatibility` keeps `Visitor` calls one hop.
...
streamed row can edge ahead of the one-shot one, because a 4 KiB window stays in L1
while a one-shot encode writes a megabyte out to memory. Their `Ir/op` gap is the
JVM's array-copy strategy folded together with the flush path, for the same reason
the Java port documents: the one-shot payload lands at offset 5, and for an
unaligned destination the JIT's copy stub takes a path Callgrind counts at roughly
one instruction per byte.

One process, ten workloads: that also costs the `decode: composite skip-all` row,
which shares a JVM with `decode: composite`, so the visitor call sites inside
`IStream` see both sinks and neither row runs monomorphic. What not-decoding is
worth shows up in `Ir/op`, where each workload gets its own JVM.
