...
IMAGE_NAME="sofab-kotlin-mp-devcontainer"
...
# Named volume holding Claude Code's configuration and credentials, so logins
# and settings survive container removal and image rebuilds.
CLAUDE_CONFIG_VOLUME="sofab-${PROJECT_NAME}-claude-config"
...
docker run -it --rm --name "$CONTAINER_NAME" \
  ...
  -v "$CLAUDE_CONFIG_VOLUME":/root/.claude \
  --workdir /workspace \
  "$IMAGE_NAME" \
  bash
